...
public class TicTacToe {
    ...
    static void compTurn() {
        ...
        int computerTurn = 0;
        computerTurn = (int) Math.floor(Math.random() * 10) % 10;
        if (computerTurn == 0) {
            computerTurn = computerTurn + 1;
        }
        System.out.println("Computer chooses the board position: " + computerTurn);
        if (board[computerTurn] == ' ') {
            board[computerTurn] = computer;
        }
    }
    ...
    static void gameCheck() {
        ...
        for (int i = 1; i < 7; i++) {
            ...
            if (i == 3) {
                if (board[i] == board[i + 2] && board[i] == board[i + 4] && board[i] == player) {
                    System.out.println("Computer Won");
                    won = 1;
                    break;
    ...
    static void toss() {
        ...
        if (toss == HEAD) {
            System.out.println("Player won toss and will start start first.");
        } else {
